...
So as soon as you start working on your project, there is one branch (~main~) and you are on it. Wait... what does this mean to be /on a branch/? There is another pointer called ~HEAD~ that keeps track of where you are. And ~HEAD~ points to ~main~.

This is what ~On branch main~ means in the output of ~git status~. \\
This is also what ~(HEAD -> main)~ means when you run ~git log~ (you need to create at least one commit before you can run ~git log~).
...
{{</img>}}
{{<br>}}
You are happy with your changes and you want to merge them back into the main branch (~main~).

First, you checkout main (to merge, you need to be on the branch you want to merge /into/):
...
{{</img>}}
{{<br>}}
This merge is called a *fast-forward merge*: the ~main~ branch could be fast-forwarded to the branch ~test~.
